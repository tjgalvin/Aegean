...
def mask_to_bin_coord(i_data):
    """
    Calculate the valid (finite) pixels and the return the equivilant pixel
    center position

    Parameters
    ----------
    im : :class:`numpy.ndarray`
        Image data to consider
    ...
    pix_pos : :class:`List[numpy.ndarray, numpy.ndarray]`
        The `x` and `y` pixel coordinate positions
...
def characterise_islands(
    ...
    for src, isle in zip(sources, islands):
        ...
        if err_type == "best":
            ...
            valid_mask, (mx, my) = mask_to_bin_coord(i_data)
            C = Cmatrix(
...
class SourceFinder(object):
    ...
    def _make_bkg_rms(
        self, filename, mesh_size=20, forced_rms=None, forced_bkg=None, cores=None
    ):
        """
    ...
    def _refit_islands(self, group, stage, outerclip=None, istart=0):
        ...
        for inum, isle in enumerate(group, start=istart):
            ...
            # TODO: Update the pixel positions here
            valid_mask, (mx, my) = mask_to_bin_coord(idata)
            ...
            for i in range(params["components"].value):
                ...
                if not np.any(np.isfinite(square)):
                    ...
                    for p in ["amp", "xo", "yo", "sx", "sy", "theta"]:
                        ...
                        params[
                            prefix + p
                        ].stderr = np.nan  # this results in an error of -1 later on
