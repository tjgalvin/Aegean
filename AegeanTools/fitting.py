...
def condon_errors(source, theta_n, psf=None):
    ...
    major = source.a / 3600.0  # degrees
    minor = source.b / 3600.0  # degrees
    ...
    if theta_n is None:
        source.err_a = (
            source.err_b
        ) = source.err_peak_flux = source.err_pa = source.err_int_flux = 0.0
        return
    ...
    source.err_a = major * np.sqrt(2 / rho2("major")) * 3600.0  # arcsec
    source.err_b = minor * np.sqrt(2 / rho2("minor")) * 3600.0  # arcsec

    err_xo2 = 2.0 / rho2("xo") * major ** 2 / (8 * np.log(2))  # Condon'97 eq 21
    err_yo2 = 2.0 / rho2("yo") * minor ** 2 / (8 * np.log(2))
...
def do_lmfit(data, params, B=None, errs=None, dojac=True):
    ...
    def residual(params, **kwargs):
        ...
        if B is None:
            return model - data[valid_mask]
        else:
            return (model - data[valid_mask]).dot(B)

    # TODO: Increase the x and y here
    if dojac:
...
if __name__ == "__main__":
    ...
    def plot_jacobian():
        ...
        params.add(
            "c0_xo",
            value=1.0 * nx / 2,
            min=nx / 2.0 - smoothing / 2.0,
            max=nx / 2.0 + smoothing / 2,
        )
        params.add(
            "c0_yo",
            value=1.0 * ny / 2,
            min=ny / 2.0 - smoothing / 2.0,
            max=ny / 2.0 + smoothing / 2.0,
        )
